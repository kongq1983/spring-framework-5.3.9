...
class CglibAopProxy implements AopProxy, Serializable {
	...
	public Object getProxy(@Nullable ClassLoader classLoader) {
		...
		try {
			...
			Class<?> proxySuperClass = rootClass; // 如果被代理类本身就已经是Cglib所生成的代理类了
			if (rootClass.getName().contains(ClassUtils.CGLIB_CLASS_SEPARATOR)) {
				proxySuperClass = rootClass.getSuperclass(); // 获取真正的被代理类
				Class<?>[] additionalInterfaces = rootClass.getInterfaces(); // 获取被代理类所实现的接口
			...
			enhancer.setSuperclass(proxySuperClass); // 被代理类，代理类的父类
			enhancer.setInterfaces(AopProxyUtils.completeProxiedInterfaces(this.advised)); // 代理类额外要实现的接口
			enhancer.setNamingPolicy(SpringNamingPolicy.INSTANCE);
			enhancer.setStrategy(new ClassLoaderAwareGeneratorStrategy(classLoader));

			Callback[] callbacks = getCallbacks(rootClass); // 获取和被代理类所匹配的Advisor
